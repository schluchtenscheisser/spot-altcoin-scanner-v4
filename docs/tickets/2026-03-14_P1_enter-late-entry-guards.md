...
- **Threshold behavior is explicit:**
  ...
  - default threshold for this ticket: reuse the existing decision intent of “current-price RR must still be worthwhile” and set the canonical default explicitly in config/docs/tests
  - missing key => documented default
