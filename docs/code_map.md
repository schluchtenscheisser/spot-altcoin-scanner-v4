...
**Last Updated:** 2026-02-27 20:58 UTC  
...
**Functions:** `_is_triggered, _parse_date, _resolve_thresholds, _threshold_key, _to_float, _trade_level_status, evaluate_e2_candidate`

**Module Variables:** `DEFAULT_THRESHOLDS_PCT, DEFAULT_T_HOLD, DEFAULT_T_TRIGGER_MAX, breakout, breakout_invalid, candle, close, day, entry_price, fallback` _(+34 more)_
...
**Module Variables:** `a_max, asks, b_max, best_ask, best_bid, bids, c_max, cfg, grade, level_quote` _(+21 more)_
...
**Functions:** `_build_price_series_by_symbol, _daterange, _load_snapshot, _parse_date, _run_id_from_asof, _score_from_entry, _utc_now_iso, build_parser, export_dataset, main`

**Module Variables:** `DATASET_SCHEMA_VERSION, args, backtest_cfg, candle, config, config_root, current, dt, e2, end` _(+31 more)_
...
| `_resolve_thresholds` | `_to_float` | `add`, `get` |
| `_threshold_key` | — | `is_integer` |
| `_trade_level_status` | `_to_float` | `get` |
| `evaluate_e2_candidate` | `_is_triggered`, `_parse_date`, `_resolve_thresholds`, `_threshold_key`, `_to_float`, `_trade_level_status` | `append`, `get`, `isoformat`, `timedelta` |
...
| `_parse_date` | — | `fromisoformat` |
| `_run_id_from_asof` | — | `strftime`, `strptime` |
| `_score_from_entry` | — | `ValueError`, `get` |
| `_utc_now_iso` | — | `now`, `strftime` |
| `build_parser` | — | `ArgumentParser`, `add_argument` |
| `export_dataset` | `_build_price_series_by_symbol`, `_daterange`, `_load_snapshot`, `_parse_date`, `_run_id_from_asof`, `_score_from_entry`, `_utc_now_iso` | `FileNotFoundError`, `Path`, `ValueError`, `append`, `compute_global_top20`, `dumps`, `evaluate_e2_candidate`, `exists`, `get`, `isoformat`, `items`, `join`, `load_config`, `mkdir`, `sort`, `write` |
...
| `scanner/tools/export_evaluation_dataset.py` | 9 | 32 | 41 | 🔴 High |
...
| `scanner/backtest/e2_model.py` | 9 | 10 | 19 | ⚠️ Medium |
...
_Generated by GitHub Actions • 2026-02-27 20:58 UTC_
