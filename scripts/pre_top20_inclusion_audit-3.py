...
-------------------------
- excludes tests/fixtures entirely
- prioritizes same-run / same-date sources
- accepts only arrays with global-ranking-capable score fields
"""
...
IGNORED_DIR_PARTS = {".git", ".venv", "venv", "__pycache__", "tests", "fixtures"}
SNAPSHOT_PRIORITY = [
    "snapshots/runtime",
    "snapshots/history",
    "snapshots/raw",
...
GLOBAL_RANKING_SCORE_KEYS = ("global_score", "weighted_score", "final_score", "setup_score")
...
def extract_run_dates(run: RunRecord) -> List[str]:
    dates = set()

    if run.timestamp_utc:
        m = re.match(r"^(\d{4}-\d{2}-\d{2})", run.timestamp_utc)
        if m:
            dates.add(m.group(1))

    for source in (run.run_id, run.path.name, run.path.stem):
        for found in re.findall(r"\d{4}-\d{2}-\d{2}", source):
            dates.add(found)

    return sorted(dates)


def extract_run_tokens(run: RunRecord) -> List[str]:
    tokens = set()
    for source in (run.run_id, run.path.name, run.path.stem):
        for found in re.findall(r"\d{6,}", source):
            tokens.add(found)
    return sorted(tokens)


def candidate_array_quality(item: Dict[str, Any]) -> int:
    ...
    if "setup_id" in keys:
        quality += 2
    for key in GLOBAL_RANKING_SCORE_KEYS:
        if key in keys:
            quality += 3
    if "tradeability_class" in keys:
        quality += 1
    if "risk_acceptable" in keys:
        quality += 1
    if "entry_ready" in keys:
        quality += 1
    return quality


def is_global_ranking_capable(item: Dict[str, Any]) -> bool:
    keys = set(item["sample_keys"])
    return "symbol" in keys and any(k in keys for k in GLOBAL_RANKING_SCORE_KEYS)
...
def detect_candidate_arrays(payload: Any, min_len: int = 21) -> List[Dict[str, Any]]:
    ...
    def inspect_container(container: Any, path: str) -> None:
        ...
        if not is_global_ranking_capable(item):
            return

        findings.append(item)

    inspect_container(payload, "")
    findings.sort(key=lambda x: (-candidate_array_quality(x), -x["length"], x["path"]))
    return findings
...
def choose_best_candidate_array(findings: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    ranked = sorted(
        findings,
        key=lambda x: (-candidate_array_quality(x), -x["length"], x["path"]),
    )
    return ranked[0]
...
def eligible_json_path(path: Path) -> bool:
    ...
    if any(part.lower() in lowered_parts for part in IGNORED_DIR_PARTS):
        return False
    ...
    if "/reports/analysis/" in path_str:
        return False
    if "/tests/" in path_str or "/fixtures/" in path_str:
        return False
...
def source_match_score(path: Path, run: RunRecord, root: Path) -> int:
    rel = str(path.resolve().relative_to(root.resolve())).replace("\\", "/")
    name = path.name
    score = 0

    for token in extract_run_tokens(run):
        if token and token in rel:
            score += 100

    for d in extract_run_dates(run):
        if d and d in rel:
            score += 25
            if name.startswith(d):
                score += 10

    run_id_fragments = [frag for frag in re.split(r"[^A-Za-z0-9]+", run.run_id) if len(frag) >= 8]
    for frag in run_id_fragments:
        if frag in rel:
            score += 50
...
def discover_upstream_json_candidates(root: Path, exclude_paths: List[Path], min_len: int, run: RunRecord) -> List[Dict[str, Any]]:
    findings: List[Dict[str, Any]] = []
            ...
            findings.append(
                {
                    ...
                    "match_score": source_match_score(path, run, root),
                    "array_path": best["path"],
                    "array_length": best["length"],
                    "sample_keys": best["sample_keys"],
                    "array_quality": candidate_array_quality(best),
                    "rows": best["rows"],
                }
            )
...
def resolve_inputs(args: argparse.Namespace) -> Tuple[RunRecord, Optional[Dict[str, Any]]]:
    ...
    if args.upstream_json:
        ...
        best = choose_best_candidate_array(arrays)
        if best is None:
            raise SystemExit(f"No suitable larger global-ranking-capable candidate array found in {args.upstream_json}")
        upstream_candidate = {
        ...
        if nearby:
            ...
            upstream_candidate = {
                ...
                "sample_keys": best["sample_keys"],
                "array_quality": best["array_quality"],
                "rows": best["rows"],
            }
...
def build_missing_snapshot_guidance(run: RunRecord) -> Dict[str, Any]:
    return {
        ...
        "message": (
            ...
            "or other repo JSONs after excluding tests/fixtures and requiring global-ranking-capable score fields."
        ),
        "search_order_used": SNAPSHOT_PRIORITY,
        "run_id_target": run.run_id,
        "run_dates_target": extract_run_dates(run),
        "run_tokens_target": extract_run_tokens(run),
        "required_minimum_fields_per_row": [
            "symbol",
            "setup_id",
            "best_setup_type or setup_type",
            "at least one of: global_score / weighted_score / final_score / setup_score",
        ],
        ...
        "preferred_snapshot_semantics": [
            "same run_id / asof context as published run JSON",
...
def main() -> int:
    ...
    if upstream is None:
        payload["missing_snapshot_guidance"] = build_missing_snapshot_guidance(run)
    else:
        payload["upstream_candidate_source"] = {
            ...
            "match_score": upstream["match_score"],
            "array_path": upstream["array_path"],
            "array_length": upstream["array_length"],
            "array_quality": upstream["array_quality"],
